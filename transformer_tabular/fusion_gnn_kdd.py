...
from tabular_transformer.dataset import load_split
...
@torch.no_grad()
def transformer_probs_from_tensors(
    ckpt_path: Path,
    tensors,
    meta: Dict,
    batch_size: int,
    ...
    # Manual batching to avoid importing DataLoader here
    x_num = tensors.x_num.to(device)
    x_cat = tensors.x_cat.to(device)
...
def build_service_adj_shared_protocol(service_ids: np.ndarray, proto_ids: np.ndarray, services: np.ndarray) -> np.ndarray:
...
def decode_vocab(vocab: Dict[str, Dict[str, str]], col: str, token_id: int) -> str:
    m = vocab.get(col, {})
    return m.get(str(int(token_id)), str(int(token_id)))


def main() -> None:
    ap = argparse.ArgumentParser()
    ap.add_argument("--data_dir", type=str, default="transformer_tabular/data/processed/kdd")
    ap.add_argument("--vocab_json", type=str, default=None, help="Optional vocab.json from preprocess_kdd.py")
    ap.add_argument("--transformer_ckpt", type=str, default="transformer_tabular/runs/kdd/best_model.pt")
    ap.add_argument("--out_dir", type=str, default="transformer_tabular/runs/fusion_kdd_gnn")
    ...
    if "cat_cols" not in meta or "protocol_type" not in meta["cat_cols"] or "service" not in meta["cat_cols"]:
        raise SystemExit(f"Expected cat_cols to include protocol_type and service. Got: {meta.get('cat_cols')}")

    proto_idx = meta["cat_cols"].index("protocol_type")
    service_idx = meta["cat_cols"].index("service")

    x_cat = train_tensors.x_cat.detach().cpu().numpy()
    y = train_tensors.y.detach().cpu().numpy().astype(np.int64)
    ...
    probs = transformer_probs_from_tensors(Path(args.transformer_ckpt), train_tensors, meta, args.infer_batch, device)

    X_svc, y_svc, counts = aggregate_per_service(services, service_ids, probs, y)

    A = build_service_adj_shared_protocol(service_ids, proto_ids, services)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % 20 == 0 or epoch == 1:
            ...
            if len(val_idx) == 0:
                val_auc = None
                val_true = np.array([], dtype=np.int64)
                val_pred = np.array([], dtype=np.int64)
            else:
                val_true = y_svc[val_idx]
                val_prob = prob_all[val_idx]
                val_pred = pred_all[val_idx]
                val_auc = None if len(np.unique(val_true)) < 2 else float(roc_auc_score(val_true, val_prob))
    ...
    report = {
        "dataset": "kdd",
        "fusion": "transformer_probs_as_features + service_graph_shared_protocol_type + SimpleGCN",
    ...
    df = pd.DataFrame(
        {
            ...
            "service": [decode_vocab(vocab, "service", int(s)) for s in services.tolist()],
    ...
    torch.save(
        {"model": model.state_dict(), "services": services, "meta": meta, "report": report},
